...
# 
# 
# fit.expr.b <- mlinear_model(expr, target, tfs.candidates)
# fit.activity.b <- mlinear_model(plotting.data.candidates, target, tfs.candidates)
# 
# fit.expr.c <- lasso_model(expr, target, tfs.candidates)
# fit.activity.c <- lasso_model(plotting.data.candidates, target, tfs.candidates)
# 
# fit.expr.d <- mombf_model(expr, target, tfs.candidates)
# fit.activity.d <- mombf_model(plotting.data.candidates, target, tfs.candidates)
...
pred.expr.c <- predict(fit.expr.c, 
        newx = t(expr[tfs.query, ]), type = "response", s = 0.05) # Gives the fitted values for the first ? observations at lambda = 0.05
